...
void* client_handler(void* arg) {
    ...
    while (!logged_in) {
        ...
        if (pkt.type == PKT_REQ_LOGIN) {
            ...
            // DB 조회 및 생성 (소각 계정 차단 포함)
            UserRecord rec;
            off_t offset;
            int found = userdb_find(pkt.body.login.key, &rec, &offset);
            if (found == 0 && userdb_is_burned(&rec)) {
                Packet res;
                memset(&res, 0, sizeof(Packet));
                res.type = PKT_RES_LOGIN_FAIL;
            ...
            if (found != 0) {
                // 신규 가입
                if (userdb_signup(pkt.body.login.key, &rec, &offset) != 0) {
                    Packet res;
                    memset(&res, 0, sizeof(Packet));
                    res.type = PKT_RES_LOGIN_FAIL;
                    res.body.error.error_code = ERR_INVALID_SESSION;
                    strncpy(res.body.error.reason, "신규 가입 실패.", MAX_TEXT_LEN - 1);
                    packet_send(sock, &res);
                    goto disconnect;
                }
            }
